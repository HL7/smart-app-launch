...
{% include info-note.html text='The overview tables and annotated examples on this page are intended for convenience and clarity. In case of any discrepancies, the formal profiles should be considered the definitive source of truth.' %}
...
Note that the presence of an Endpoint in the Brand Bundle does not provide an implicit authorization to access the Endpoint. Clients that require access to the data provided by the FHIR Endpoints in the Brand Bundle should inspect the SMART Configuration of the Endpoint in order to retrieve information on the requirements for accessing this server.
